...
var validY={};
//验证手机号
validY.validPhone=function (tel) {
    console.log(tel);
    var reg=(/^1[34578]\d{9}$/.test(tel));

    return reg ? '0000' : '手机号码格式错误';
}
//验证身份证号
validY.validIdCard=function (card) {
    var result='0000';
...
//验证银行卡号
validY.validBankCard=function (e){
    if(!/^\d{16}|\d{19}$/.test(e)){
        return "银行卡号输入错误";
    }else{
        return "0000";
...
//验证账单条形码
validY.validCateCode=function (e){
    if(e==""){
        return "请输入账单条形码";
    }else if(!/^\d*$/i.test(e)){
